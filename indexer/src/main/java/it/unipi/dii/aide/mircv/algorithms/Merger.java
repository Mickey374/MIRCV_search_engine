...
import it.unipi.dii.aide.mircv.beans.BlockDescriptor;
...
public class Merger {
     ...
     */
    private static String PATH_TO_PARTIAL_INDEXES_DOCS = ConfigurationParams.getDocidsDir() + ConfigurationParams.getDocidsFileName();

    /**
     * Standard pathname for partial index frequencies files
     */
    private static String PATH_TO_PARTIAL_INDEXES_FREQS = ConfigurationParams.getFrequencyDir() + ConfigurationParams.getFrequencyFileName();

    /**
     * Standard pathname for partial vocabulary files
     */
    private static String PATH_TO_PARTIAL_VOCABULARIES = ConfigurationParams.getPartialVocabularyDir() + ConfigurationParams.getVocabularyFilename();

    /**
     * Path to the inverted index docs file
     */
    private static String PATH_TO_INVERTED_INDEX_DOCS = ConfigurationParams.getInvertedIndexDocs();

    /**
     * Path to the inverted index freqs file
     */
    private static String PATH_TO_INVERTED_INDEX_FREQS = ConfigurationParams.getInvertedIndexFreqs();

    /**
     * Path to vocabulary
     */
    private static String PATH_TO_VOCABULARY = ConfigurationParams.getVocabularyPath();

    /**
     * path to block descriptors file
     */
    private static String PATH_TO_BLOCK_DESCRIPTORS = ConfigurationParams.getBlockDescriptorsPath();
    ...
    private static PostingList processTerm(String termToProcess, VocabularyEntry vocabularyEntry) {
        ...
        vocabularyEntry.setMemoryOffset((int) docsMemOffset);
                        ...
                        while(true){
                            ...
                            if (postingsInBlock == nPostingsToBeWritten){
                                ...
                                try{
                                    ...
                                    blockDescriptor.saveBlockDescriptorOnDisk(descriptorChan);
